# update_data.py ← FINAL, CLEAN, WORKING VERSION (Aug 2025)
...
def get_key_events():
    ...
    if next_rbi:
        ...
        events.append({"Timeframe":"Upcoming","Event":"Next RBI Policy","Value":f"{next_rbi.strftime('%b %d')} ({days} days)","Impact":"Very High","Details":get_related_news("RBI policy",7)})
    ...
    # India CPI (12th of every month)
    cpi = (now.replace(day=1) + timedelta(days=40)).replace(day=12)
    if cpi <= now:
        cpi = (cpi.replace(day=1) + timedelta(days=40)).replace(day=12)
    days = (cpi - now).days
    events.append({"Timeframe":"Upcoming","Event":"Next India CPI","Value":f"{cpi.strftime('%b %d')} ({days} days)","Impact":"High","Details":get_related_news("India CPI",7)})
